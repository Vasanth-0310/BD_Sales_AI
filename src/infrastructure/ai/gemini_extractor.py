...
from src.domain.value_objects.job_details import (
    JobDetails,
    SalaryInfo,
...
def _validate_salary(job_details: "JobDetails") -> "JobDetails":
    """Deterministic salary validation/correction.

    Re-parses the raw salary string (`salary_info.raw`, Gemini's verbatim
    copy of the posting text) with regex and cross-checks Gemini's other
    structured fields against it. Numbers and period/currency from the raw
    text ALWAYS win — this catches mislabeled periods (e.g. a
    $171,000-$205,000 ANNUAL range tagged "Hourly") and hallucinated
    figures. If the raw string is unparseable, Gemini's values stand.
    """
    raw = ((job_details.salary_info.raw if job_details.salary_info else "") or "").strip()
    if not raw:
    ...
    words = {"usd": "USD", "inr": "INR", "eur": "EUR", "gbp": "GBP",
             "cad": "CAD", "aud": "AUD", "dollar": "USD", "dollars": "USD",
             "rupee": "INR", "rupees": "INR", "euro": "EUR", "euros": "EUR"}
    currency = None
    for sym, iso in symbols.items():
        if sym in raw:
            currency = iso
            break
    if currency is None:
        for w, iso in words.items():
            if re.search(rf"\b{w}\b", raw, re.I):
                currency = iso
                break

    # ── Parse period (shared keyword map lives in the domain layer) ────
    ...
    if existing is not None:
        # Keep Gemini's values ONLY where the raw text couldn't confirm ours.
        merged = dict(info)
        if existing.currency and currency is None:
            merged["currency"] = existing.currency
        if existing.pay_period and period is None:
            merged["pay_period"] = existing.pay_period
        # If Gemini's numbers disagree with the raw text, parsed wins silently
        # (already the case — merged carries parsed values).
        return job_details.model_copy(update={"salary_info": SalaryInfo(**merged)})

    return job_details.model_copy(update={"salary_info": SalaryInfo(**info)})


def jd_drop_salary_info(job_details: "JobDetails") -> "JobDetails":
    """Null out salary_info when there is no raw salary text to support it."""
    if job_details.salary_info is None:
        return job_details
    return job_details.model_copy(update={"salary_info": None})
...
class GeminiExtractor(IExtractor):
    ...
    async def extract(self, cleaned_text: str) -> JobDetails:
        ...
        try:
            ...
            job_details = _validate_salary(
                _sanitize_proposal_questions(
