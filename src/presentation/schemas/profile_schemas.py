...
class ProfileMatchRequest(BaseModel):
    ...
    variant_id: Optional[str] = Field(
        default=None,
        description=(
            "Optional. UUID of a specific profile variant. "
            "When provided, skips vector search and fetches this variant "
            "directly from Qdrant for Gemini scoring. "
            "Null or empty string triggers the normal auto-match pipeline."
        ),
    )
